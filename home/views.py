...
from .models import CurrentIpoName, GroupDetail, Order, OrderDetail, ClientDetail, CustomUser, RateList, Accounting, SharedLink, AccountingAuditLog
...
import pandas as pd
from django.db.models import Avg, Max, Min, Sum, Exists, OuterRef, Q, CharField, F, Case, When, Value, DecimalField, FloatField, Count
...
def get_dynamic_filter_options(
    ...
    selected_investor_type=None,
    prefix=""
):
    """
    Returns only the Groups, OrderCategories, and InvestorTypes 
    that actually have orders in `orders_qs`.
    """
    ALL_CATEGORIES = ['Kostak', 'Subject To', 'CALL', 'PUT', 'Premium']
    INVESTOR_TYPE_MAP = [
        ('RETAIL', 'Retail'),
        ('SHNI', 'SHNI'),
        ('BHNI', 'BHNI'),
    ...
    # 1. Distinct Groups that have orders
    group_col = f"{prefix}OrderGroup_id"
    order_group_ids = orders_qs.exclude(**{f"{prefix}OrderGroup": None}).values_list(group_col, flat=True).distinct()
    Group = GroupDetail.objects.filter(user=userid, id__in=order_group_ids)
    if is_valid_queryparam(selected_group) and selected_group != 'All':
        Group = (Group | GroupDetail.objects.filter(user=userid, GroupName=selected_group)).distinct()

    # 2. Distinct Categories
    cat_col = f"{prefix}OrderCategory"
    raw_cats = set(orders_qs.exclude(**{cat_col: None}).values_list(cat_col, flat=True).distinct())
    normalized_cats = {'Subject To' if c == 'SubjectTo' else c for c in raw_cats if c}
    if is_valid_queryparam(selected_category) and selected_category != 'All':
        normalized_cats.add(selected_category)
    available_categories = [c for c in ALL_CATEGORIES if c in normalized_cats]
    for c in sorted(normalized_cats):
        if c not in available_categories:
            available_categories.append(c)

    # 3. Distinct Investor Types
    inv_col = f"{prefix}InvestorType"
    raw_invs = set(orders_qs.exclude(**{inv_col: None}).values_list(inv_col, flat=True).distinct())
    raw_invs_upper = {inv.upper() for inv in raw_invs if inv}
    if is_valid_queryparam(selected_investor_type) and selected_investor_type != 'All':
        raw_invs_upper.add(selected_investor_type.upper())
    available_investor_types = [(val, label) for val, label in INVESTOR_TYPE_MAP if val in raw_invs_upper]
    for inv in sorted(raw_invs):
        if inv and inv.upper() not in [val for val, _ in INVESTOR_TYPE_MAP]:
            available_investor_types.append((inv, inv))

    return Group, available_categories, available_investor_types
...
def OrderDetailFunction(request, IPOid, Ordtyp, GrpName=None, OrderCategory=None, InvestorType=None, OrderDate=None, OrderTime=None,Rate='All'):
    ...
    if request.method == "POST":
        if has_session_access:
            Groupfilter = unquote(GrpName) if GrpName else 'All'
            RateFilterValue = Rate if Rate != 'None' and Rate is not None else 'All'
        else:
            Groupfilter = request.POST.get('Groupfilter', 'All') or 'All'
            RateFilterValue = request.POST.get('RateFilterValue', 'All') or 'All'
        IPOTypefilter = request.POST.get('IPOTypefilter', 'All') or 'All'
        InvestorTypeFilter = request.POST.get('InvestorTypeFilter', 'All') or 'All'
    else:
        Groupfilter = unquote(GrpName) if GrpName else 'All'
        IPOTypefilter = unquote(OrderCategory) if OrderCategory else 'All'
        InvestorTypeFilter = InvestorType if InvestorType else 'All'
        RateFilterValue = Rate if Rate != 'None' and Rate is not None else 'All'
    ...
    if RateFilterValue == '' or RateFilterValue is None or RateFilterValue == 'None':
        RateFilterValue = 'All'

    # Dynamic filter options from base orders
    if has_session_access:
        user = request.session[f'link_owner_{IPOid}']
        link_id = request.session[f'access']
        Group_id = SharedLink.objects.get(id=link_id).group
        Group = GroupDetail.objects.filter(user=user, GroupName=Group_id)
        _, available_categories, available_investor_types = get_dynamic_filter_options(
            entry, user, Groupfilter, IPOTypefilter, InvestorTypeFilter, prefix="Order__"
        )
    else:
        filter_user = request.user if request.user.groups.all()[0].name == 'Broker' else request.user.Broker_id
        Group, available_categories, available_investor_types = get_dynamic_filter_options(
            entry, filter_user, Groupfilter, IPOTypefilter, InvestorTypeFilter, prefix="Order__"
        )
    ...
    if is_valid_queryparam(IPOTypefilter) and IPOTypefilter != 'All':
        if IPOTypefilter == 'Subject To':
            entry = entry.filter(Q(Order__OrderCategory='Subject To') | Q(Order__OrderCategory='SubjectTo'))
        else:
            entry = entry.filter(Order__OrderCategory=IPOTypefilter)
    if is_valid_queryparam(InvestorTypeFilter) and InvestorTypeFilter != 'All':
        entry = entry.filter(Order__InvestorType__iexact=InvestorTypeFilter)
    ...
    if request.method == "POST" and OrderDate is not None and OrderTime is not None:
...
def filterfromstatus(request, IPOid, Groupfilter, OrderCategoryFilter, InvestorTypeFilter):
    ...
    Group, available_categories, available_investor_types = get_dynamic_filter_options(
        products, userid, Groupfilter, OrderCategoryFilter, InvestorTypeFilter
    )
    ...
    if is_valid_queryparam(OrderCategoryFilter) and OrderCategoryFilter != 'All':
        if OrderCategoryFilter == 'Subject To':
            products = products.filter(Q(OrderCategory='Subject To') | Q(OrderCategory='SubjectTo'))
        else:
            products = products.filter(OrderCategory=OrderCategoryFilter)
    if is_valid_queryparam(InvestorTypeFilter) and InvestorTypeFilter != 'All':
        products = products.filter(InvestorType__iexact=InvestorTypeFilter)
...
def GroupWiseDashboard(request):
    ...
    if SearchQuery:
        Group = Group.filter(GroupName__icontains=SearchQuery)
    ...
    for index, row in df.iterrows():
        ...
        for col_name, cell in row.items():
            if col_name != 'JV' and col_name != 'Total' and col_name != 'Collection' and col_name != 'Due Amount' and col_name != 'New Collection':
                ...
                html_table += (
                    f'<td class="amount-cell" data-ipo-id="{ipo.id}" data-ipo-name="{col_name}" data-group-id="{index.id}" data-group-name="{index}" data-exact-due="{due_amount}">'
                    f'<span class="cell-total" title="Double click to view Client Wise Billing">Total: {float_format.format(cell)}</span><br>'
                    f'<span class="cell-collection" title="Double click to view Accounting entries">Collection: {float_format.format(accounting_amount1)}</span><br>'
                    f'<span class="cell-due" title="Double click to Pay Due">Due: {float_format.format(due_amount)}</span>'
                    f'</td>'
                )
    ...
    return render(request, 'GroupWiseDashboard.html', {'entry_sorted':entry_sorted,'entry': grpname, 'lenofipo': l,"ipos": ipos, "groups": groups, 'IPOName': IPOName, 'html_table': html_table,'IPOAmount':IPOAmount,'Total':Total,'SumCollection':SumCollection,'DueAmountSum':DueAmountSum,'page_obj': page_obj,'GWD_page_size':page_size, 'SearchQuery': SearchQuery})
...
def Billing(request, IPOid):
    ...
    Groupfilter = req_data.get('Groupfilter', 'All') or 'All'
    IPOTypefilter = req_data.get('IPOTypefilter', 'All') or 'All'
    InvestorTypeFilter = req_data.get('InvestorTypeFilter', 'All') or 'All'

    if request.user.groups.all()[0].name == 'Broker':
        billing_orders = Order.objects.filter(user=userid, OrderIPOName_id=IPOid)
    else:
        billing_orders = Order.objects.filter(user=userid, OrderIPOName_id=IPOid, OrderGroup_id=request.user.Group_id)

    Group, available_categories, available_investor_types = get_dynamic_filter_options(
        billing_orders, userid, Groupfilter, IPOTypefilter, InvestorTypeFilter
    )

    if is_valid_queryparam(Groupfilter) and Groupfilter != 'All':
        group_obj = GroupDetail.objects.filter(
            GroupName=Groupfilter, user=userid).first()
        if group_obj:
            entry = entry.filter(Order__OrderGroup_id=group_obj.id)
            order = order.filter(OrderGroup_id=group_obj.id)
    if is_valid_queryparam(IPOTypefilter) and IPOTypefilter != 'All':
        entry = entry.filter(Order__OrderCategory=IPOTypefilter)
...
def trades_page(request):
    ...
    Group, available_categories, available_investor_types = get_dynamic_filter_options(
        todays_orders, userid, Groupfilter, OrderCategoryFilter, InvestorTypeFilter
    )
    ...
    if OrderCategoryFilter != 'All':
        if OrderCategoryFilter == 'Subject To':
            todays_orders = todays_orders.filter(Q(OrderCategory='Subject To') | Q(OrderCategory='SubjectTo'))
        else:
            todays_orders = todays_orders.filter(OrderCategory=OrderCategoryFilter)
    if InvestorTypeFilter != 'All':
        todays_orders = todays_orders.filter(InvestorType__iexact=InvestorTypeFilter)
    ...
    context = {
        ...
        'available_investor_types': available_investor_types,
        'IPOFilter': IPOFilter,
        'Groupfilter': Groupfilter,
        'OrderCategoryFilter': OrderCategoryFilter,
        'InvestorTypeFilter': InvestorTypeFilter,
